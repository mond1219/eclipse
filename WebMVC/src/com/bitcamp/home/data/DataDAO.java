...
public class DataDAO extends DBCPConn implements DataDAOImpl {
	...
	public int dataInsert(DataVO vo) {
		...
		try {
			...
			System.out.println("DB에 자료실 데이터 추가 실패ㅜ");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public void dataSelect(DataVO vo) {
		try {
			getConn();
			sql="select no, title, content, userid, hit, downcount, writedate, filename1, filename2 "+
					" from data where no="+vo.getNo();
			pstmt = con.prepareStatement(sql);
			...
			System.out.println("데이터 레코드 정보 가져오기 실패,,,");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public int dataUpdate(DataVO vo, List<String> newFile) {
		...
		try {
			...
			System.out.println("DAO결과Userid---->"+vo.getUserid());
			
			result = pstmt.executeUpdate();
			
		}catch(Exception e) {
			System.out.println("자료실 글수정 실패ㅜㅜDAO");
			e.printStackTrace();
		}finally {
			getClose();
		}
...
//				vo.setFilename2(rs.getString(5));
			...
			System.out.println("자료실 전체목록 불러오기 실패ㅜ");
			e.printStackTrace();
		}finally {
			getClose();
		}
	...
	public int downloadCount(int no) {
		...
		try{
			...
			//다운로드 구하기 - select
			sql ="select downcount from data where no="+no;
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()){
				count = rs.getInt(1);
			...
			System.out.println("다운로드 회수증가 실패");
			e.printStackTrace();
		}finally {
			getClose();
		}
