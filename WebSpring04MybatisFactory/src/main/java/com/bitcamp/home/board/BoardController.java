...
public class BoardController {
	...
	public String boardView(int no, Model model) {//ModelAndView가 아닌 Model로 해보기
		BoardDAOImp dao = sqlSession.getMapper(BoardDAOImp.class);
	...
	public ModelAndView boardEditOk(BoardVO vo,HttpSession session) {
		BoardDAOImp dao = sqlSession.getMapper(BoardDAOImp.class);
		int result = dao.boardUpdate(vo.getNo(), vo.getSubject(), vo.getContent(), (String)session.getAttribute("logId"));
		
		ModelAndView mav = new ModelAndView();
		mav.addObject("no",vo.getNo());
		if(result>0) {//수정성공--글내용보기
	...
	public ModelAndView boardDelete(int no, HttpSession session) {
		BoardDAOImp dao = sqlSession.getMapper(BoardDAOImp.class);
		int result = dao.boardDelete(no,(String)session.getAttribute("logId"));
		ModelAndView mav = new ModelAndView();
		if(result>0) {//삭제성공
			mav.setViewName("redirect:boardList");
		}else {
