...
public class ImageGraphics extends JFrame implements ActionListener{
	JPanel pane = new JPanel();
		JButton btn1 = new JButton("원본그리기");
		JButton btn2 = new JButton("축소그리기");
		JButton btn3 = new JButton("확대 그리기");
		JButton btn4 = new JButton("좌우 뒤집기");
		JButton btn5 = new JButton("상하 뒤집기");
	...
	public class MyCanvas extends Canvas{
		...
		int h;
		public MyCanvas() {
			img = Toolkit.getDefaultToolkit().getImage("icon/cut01.gif");
		...
		public void paint(Graphics s) {
			...
			h = img.getHeight(this);
			//이미지를 그림으로 그리기
			if(selBtn.equals("원본 그리기")) {
				//           canvas          image
				g.drawImage(img,0,0,w,h    ,0,0,w,h,  this);
			}else if(selBtn.equals("축소 그리기")) {
				g.drawImage(img,0,0,w/2,h/2    ,0,0,w,h,  this); //1/4크기이다.
			}else if(selBtn.equals("확대 그리기")) {
				//캔버스 크기를 구해서 캔버스 크기만하게 그리기 
				int cw = getWidth(); //캔버스 폭
				int ch = getHeight();//캔버스 높이
				g.drawImage(img,0,0,cw,ch    ,0,0,w,h,  this);
			}else if(selBtn.equals("좌우 뒤집기")) {
				g.drawImge(img, w,0, 0, h, 0, 0, w, h, this);
			}else if(selBtn.equals("상하 뒤집기")) {
