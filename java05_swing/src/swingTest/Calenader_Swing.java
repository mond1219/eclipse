...
//Calenader_Swing 주석달기 오늘까지 제출
public class Calenader_Swing extends JPanel implements ItemListener,ActionListener{
...
//		super("달력");
		setLayout(new BorderLayout()); //f->B레이아웃으로 변경
...
//		setDefaultCloseOperation(DISPOSE_ON_CLOSE);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(obj ==prevBtn) {//이전 월 
			prevMonth();
		}else if(obj ==nextBtn) {//다음 월
			nextMonth();
		}
	}
	//다음월 메소드
	public void nextMonth() {
		//현재 멤버변수에 콤보박스에 있는 년과 월을 가지고 있다. 
				if(month==12) {
					year++;
					month = 1;
